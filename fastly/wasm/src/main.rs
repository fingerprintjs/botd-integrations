...
fn main(mut req: Request) -> Result<Response, Error> {
    ...
    return match req.get_path() {
        ...
        path if path == format!("/{}/detect", PATH_HASH) => {
            log::debug!("[main] Request to /{}/detect", PATH_HASH);
            ...
            let body = resp_clone.into_body().into_string();
            let request_id = match BotDetector::extract_request_id(&body) {
                Some(r) => r,
                _ => {
                    log::error!("Can't extract request id from body.");
                    String::from("")
                }
            };
        ...
        path if path.starts_with(format!("/{}/script", PATH_HASH).as_str()) => {
            log::debug!("[main] Request to /{}/script", PATH_HASH);
            return Ok(req
                .with_path("/")
                .with_pass(false)
