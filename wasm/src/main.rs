...
fn main(mut req: Request) -> Result<Response, Error> {
    ...
    match req.get_path() {
        "/" => {
            ...
            let script_connect = "<script async src=\"./dist/botd.umd.min.js\" onload=\"getResults()\"></script>";
            ...
            req.set_ttl(60);
            let result = req.send(NGROK_BACKEND).unwrap();
            let body_str = result.into_body_str();
            return Ok(Response::from_status(StatusCode::OK)
                .with_content_type(mime::TEXT_HTML_UTF_8)
                .with_body(body_str + "FingerprintJS")
                .with_header("TEST", "test"));
        }
        "/img/favicon.ico" => {
